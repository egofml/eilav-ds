...
test('3000 products: requests, isolated malformed item, stop, durable restore, resume, approve, 19-column export',async()=>{
 ...
 const block=DS.exportBlock(table,rows);assert.equal(block.length,3000);assert(block.every(r=>r.length===19));assert.equal(block[241][0],'주방 수납 바구니');assert.equal(block[0][0],'주방 정리용 수납 바구니');assert.equal(block[0][1],'1234');assert.equal(block[1][11],'0');assert.equal(block[2][0],'모형 만 14세 이상');
