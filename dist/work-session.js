...
function encode({source,options,rows}){return {version:1,source,options,edits:rows.map(r=>({checks:r.keywordChecks||[],spacingTitle:r.spacingTitle,status:r.status,name:r.approvedName,evidence:r.evidence||'',error:r.aiError,applyError:r.applyError,manualReview:r.manualReview})),savedAt:new Date().toISOString()};}
function decode(data,DS,G){
 ...
 if(edit.status==='approved'&&(!row.titleHold||edit.manualReview===true))row=DS.approve(row,edit.name,typeof edit.evidence==='string'?edit.evidence:'',o.terms,matcher,edit.manualReview===true);
 else if(edit.status==='held')row={...row,status:'held',evidence:typeof edit.evidence==='string'?edit.evidence:''};
 if(typeof edit.applyError==='string'&&row.status!=='approved')row.applyError=edit.applyError.slice(0,1000);if(typeof edit.error==='string')row.aiError=edit.error.slice(0,1000);return row;});
