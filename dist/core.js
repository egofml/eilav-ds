...
function spacingEligible(title){return !/\s/u.test(title.trim())&&(title.match(/[가-힣]/gu)||[]).length>=12;}
function checkedSpacing(title,spaced){
 if(spaced===undefined||spaced===title)return title;
 if(!spacingEligible(title)||typeof spaced!=='string'||spaced.length>title.length*2||spaced.trim()!==spaced||/[\t\r\n]/u.test(spaced)||spaced.replace(/ /g,'')!==title||!spaced.includes(' '))throw Error('특수 상품명의 공백 보정이 올바르지 않습니다.');
 // Preserve contiguous numbers, Latin model IDs and punctuation-bearing specifications.
 for(const token of title.match(/[A-Za-z0-9][A-Za-z0-9.,/_()-]*/g)||[])if(!spaced.includes(token))throw Error('규격·모델 번호 내부를 띄울 수 없습니다.');
 return spaced;
}
function applyKeywordReview(row,checks,position,spacingTitle){const base=checkedSpacing(row.cleaned,spacingTitle);if(row.exclusion?.length)throw Error("삭제 필요 검토 대상입니다.");const excluded=checks.filter(c=>c.status!=='no_obvious_issue'||position==='ai'&&c.afterWord===null).map(c=>c.keyword);const remaining=row.candidates.filter(k=>!excluded.includes(k));const proposed=position==='ai'?placeAIKeywords(base,checks):placeKeywords(base,remaining,position);return{...row,spacingTitle:base!==row.cleaned?base:undefined,aiError:undefined,keywordChecks:checks,excludedKeywords:excluded,proposed,approvedName:undefined,reviewedAt:undefined,evidence:'',status:proposed!==row.cleaned?'pending':'cleaned',empty:!row.cleaned.trim()};}
...
function readyToApply(row){return !row.blank&&!row.exclusion?.length&&row.status==='pending'&&!row.empty&&!row.aiError&&row.candidates.length>0&&row.keywordChecks.length===row.candidates.length&&row.candidates.every(k=>row.keywordChecks.filter(c=>c.keyword===k).length===1);}
...
const api={readyToApply,approveBatch,spacingEligible,checkedSpacing,exclusionReasons,HEADERS,COPY_HEADERS,FREE_COPY_HEADERS,copyHeaders,exportBlock,parseTSV,stringify,tableFrom,normalize,termsFrom,makeMatcher,fee,placeKeywords,placeAIKeywords,applyKeywordReview,analyze,approve,exportRows};if(typeof module!=='undefined'&&module.exports)module.exports=api;else root.DS=api;
