...
function applyKeywordReview(row,checks,position){const excluded=checks.filter(c=>c.status!=='no_obvious_issue').map(c=>c.keyword);const remaining=row.candidates.filter(k=>!excluded.includes(k));const proposed=placeKeywords(row.cleaned,remaining,position);return{...row,keywordChecks:checks,excludedKeywords:excluded,proposed,approvedName:undefined,reviewedAt:undefined,evidence:'',status:proposed!==row.cleaned?'pending':'cleaned',empty:!row.cleaned.trim()};}
function analyze(table,options){const count=options.keywordCount??1;if(!Number.isInteger(count)||count<1||count>3)throw Error('추가 키워드는 1~3개로 선택해주세요.');const ix=Object.fromEntries(table.names.map((n,i)=>[n,i]));const clean=makeMatcher(options.terms||[]);return table.rows.map((source,i)=>{
 ...
 const before=source[ix['상품명']],rawKeywords=source[ix['키워드']];
 ...
 const candidateReason=candidates.length?'':!rawKeywords.trim()?'키워드 없음':!terms.length?'금지어 제거 후 키워드 없음':terms.every(t=>compact(titleResult.value).includes(compact(t)))?'키워드가 이미 상품명에 포함됨':'추가 가능한 키워드 없음 (중복·길이 조건)';const keyword=candidates.join(', ');const proposed=clean(placeKeywords(titleResult.value,candidates,options.position)).value;
 ...
 return{index:i,source,output,blank:false,code:source[ix['상품코드']],original:before,cleaned:titleResult.value,proposed,keyword,candidates,candidateReason,keywordChecks:[],excludedKeywords:[],keywords,removed,issues,status:proposed!==titleResult.value?'pending':'cleaned',empty:!titleResult.value.trim(),evidence:''};
 });}
function approve(row,name,evidence,terms){const clean=makeMatcher(terms);if(!name.trim())throw Error('상품명은 비울 수 없습니다.');if(name.length>200)throw Error('상품명은 200자 이내로 입력해주세요.');if(clean(name).removed.length)throw Error('금지어가 남아 있습니다. 제거 후 승인해주세요.');if((row.excludedKeywords||[]).some(k=>normalize(name).includes(normalize(k))))throw Error('의심 또는 확인 필요로 분류된 추가 키워드는 이름에 넣을 수 없습니다.');if(/^[=+\-@]/.test(name.trim()))throw Error('수식으로 해석될 수 있는 시작 문자를 제거해주세요.');return{...row,approvedName:name.trim(),status:'approved',evidence,reviewedAt:new Date().toISOString(),empty:false};}
...
const api={HEADERS,parseTSV,stringify,tableFrom,normalize,termsFrom,makeMatcher,fee,placeKeywords,applyKeywordReview,analyze,approve,exportRows};if(typeof module!=='undefined'&&module.exports)module.exports=api;else root.DS=api;
