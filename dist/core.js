...
function analyze(table,options){const count=options.keywordCount??1;if(!Number.isInteger(count)||count<1||count>3)throw Error('추가 키워드는 1~3개로 선택해주세요.');const ix=Object.fromEntries(table.names.map((n,i)=>[n,i]));const clean=makeMatcher(options.terms||[]);return table.rows.map((source,i)=>{
 ...
 const keyword=candidates.join(', ');const proposed=clean(placeKeywords(titleResult.value,candidates,options.position)).value;
 ...
 return{index:i,source,output,blank:false,code:source[ix['상품코드']],original:before,cleaned:titleResult.value,proposed,keyword,candidates,keywordChecks:[],excludedKeywords:[],keywords,removed,issues,status:proposed!==titleResult.value?'pending':'cleaned',empty:!titleResult.value.trim(),evidence:''};
 });}
