...
function analyze(table,options){const ix=Object.fromEntries(table.names.map((n,i)=>[n,i]));const clean=makeMatcher(options.terms||[]);return table.rows.map((source,i)=>{
 ...
 const keyword=terms.find(t=>t.length>=2&&!normalize(titleResult.value).includes(normalize(t)))||'';
 const words=titleResult.value.split(/\s+/);let proposed=titleResult.value;if(keyword&&proposed){if(options.position==='start')proposed=keyword+' '+proposed;else if(options.position==='afterFirst')proposed=[words[0],keyword,...words.slice(1)].join(' ');else proposed+=' '+keyword;}proposed=clean(proposed).value;
 ...
 return{index:i,source,output,blank:false,code:source[ix['상품코드']],original:before,cleaned:titleResult.value,proposed,keyword,keywords,removed,issues,status:proposed!==titleResult.value?'pending':'cleaned',empty:!titleResult.value.trim(),evidence:''};
 });}
function approve(row,name,evidence,terms){const clean=makeMatcher(terms);if(!name.trim())throw Error('상품명은 비울 수 없습니다.');if(name.length>200)throw Error('상품명은 200자 이내로 입력해주세요.');if(clean(name).removed.length)throw Error('금지어가 남아 있습니다. 제거 후 승인해주세요.');if(/^[=+\-@]/.test(name.trim()))throw Error('수식으로 해석될 수 있는 시작 문자를 제거해주세요.');return{...row,approvedName:name.trim(),status:'approved',evidence,reviewedAt:new Date().toISOString(),empty:false};}
...
const api={HEADERS,parseTSV,stringify,tableFrom,normalize,termsFrom,makeMatcher,fee,analyze,approve,exportRows};if(typeof module!=='undefined'&&module.exports)module.exports=api;else root.DS=api;
